...
def ssd_anime_face_detect_from_PIL(image):
    ...
    if not open_cv_image.any():
        print("Error in face detect-faces converting PIL to cv2")
        return None

    # Face-detect
    return ssd_anime_face_detect_from_cv2_Image(open_cv_image, model_path)
...
def ssd_anime_face_detect(image_path, model_path):
    ...
    return ssd_anime_face_detect_from_cv2_Image(imgOrig, model_path)

def ssd_anime_face_detect_from_cv2_Image(imgOrig, model_path):
    cfg = {
        'name': 'FaceBoxes',
        #'min_dim': 1024,
